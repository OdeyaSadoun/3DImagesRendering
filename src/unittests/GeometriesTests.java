...
public class GeometriesTests 
{
	...
	public void testAdd() 
	{
		try 
		{
			...
			assertEquals("the length of the list is worng", 3, collection.getIntsersectionPoints().toArray().length);
	...
	public void testFindIntsersections() 
	{
		...
		try 
		{
			...
			assertNull("No cut shape must return 0",collection.findIntersections(new Ray(new Point3D(10.45, 1.89, 0), new Vector(-20.24,-0.2,0))));
			...
			collection= new Geometries(sphere, triangle/*, tube ,cylinder*/);
			assertEquals("wrong number of intersactions", 3, collection.findIntersections(new Ray(new Point3D(-4, 6, 0), new Vector(2.13,-7.34,0))).size());
			...
			collection= new Geometries(sphere, triangle/*, tube ,cylinder*/);
			assertEquals("wrong number of intersactions", 2, collection.findIntersections(new Ray(new Point3D(1.87, 8.49, 0), new Vector(-1.54,-16.82,0))).size());
